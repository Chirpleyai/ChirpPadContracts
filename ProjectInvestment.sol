 ...
 */
contract ProjectInvestment is Ownable, ReentrancyGuard {
     ...
     */
    function investRound1(uint256 amount) external nonReentrant {
        require(amount > 0, "Amount must be greater than 0");
        require(!round1Complete, "Round 1 is complete");
        require(round1.totalInvestment + amount <= round1Target, "Exceeds round 1 target");

        uint256 userAllocation = round1.maxAllocations[msg.sender];
        require(userAllocation > 0, "User has no allocation");
        require(round1.investments[msg.sender] + amount <= userAllocation, "Exceeds max allocation");
     ...
     * @param amount The amount of tokens the user wishes to invest.
     */
    function investRound2(uint256 amount) external nonReentrant {
        require(amount > 0, "Amount must be greater than 0");
        ...
        require(round2.investments[msg.sender] + amount <= userAllocation, "Exceeds max allocation for Round 2");
     ...
     */
    function batchSetRound1Allocation(address[] calldata users, uint256[] calldata allocations) external onlyOwner {
        require(users.length == allocations.length, "Users and allocations length mismatch");

        uint256 newTotalAllocation = round1.totalAllocation;

        for (uint256 i = 0; i < users.length; i++) {
            require(users[i] != address(0), "User address cannot be zero");
            require(allocations[i] > 0, "Allocation must be greater than 0");
            require(allocations[i] <= round1Target, "Individual allocation exceeds round1Target");
            ...
            require(newTotalAllocation <= round1Target, "Total allocation exceeds round1Target");
    ...
    function setRound2MaxAllocation(uint256 maxAllocation) external onlyOwner {
        ...
        require(maxAllocation <= remainingAllocation, "Max allocation exceeds remaining allocation from Round 1");
